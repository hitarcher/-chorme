...
//#define TIMER_LOADOFFLINE   1009
// #define templateONLINE	"template\\templateonline.json"
// #define templateOFFLINE "template\\templateoffline.json"
//#define OFFLINETIMELIMTE 10*60
//BOOL g_bIsOnLinePro = TRUE;										//判断是否是在线节目
//UINT g_nProgameMode = 0;
...
void CWeChatPrinterDlg::LoadTemplate()
{
	...
	//CString strFilePath = GetFullPath(g_Config.GetStringFromCfgDev("BASE", "Template")+"template/template.json");
// 	CString strFilePath = GetFullPath(g_Config.GetStringFromCfgDev("BASE", "Template") + g_Config.GetStringFromCfgDev("BASE", "TempalteJson"));	
// 	//strContent.Format("{\"body\":{\"templatejson\":%s}}",ReadFileContent(strFilePath.GetBuffer(0)));
// 	strContent.Format("%s", ReadFileContent(strFilePath.GetBuffer(0)));
// 	json jTemp = json::parse(strContent.GetBuffer(0));
	...
	// 仅仅做解析
	//ParseTemplateOfH5(ReadFileContent(strFilePath.GetBuffer(0)), TRUE);
	//SHOW_TIPS(TIPS_WELCOME);
	//LOG(LOGTYPE_DEBUG, LOG_NAME_DEBUG, "LoadTemplate", "[LoadMainFrame()][out]");
...
BOOL CWeChatPrinterDlg::CheckUpdate()
{
	...
	if (FALSE == CheckFileExist(strMsgPath))
	{
		...
		//StartProcess2(strPath, FALSE);//2020.12.29 失效了，打开闪退
	...
	if (vct.size() >= 1)
	{
		if (vct[0].CompareNoCase("PUBLISH") == 0 && vct.size() == 4)
		{
			...
			if (bRet)
			{
				...
				bRet = g_toolTrade.UpLoadProGrameStatus(g_Config.m_strHttpUrl.GetBuffer(0), m_nMode, 2);
				if (bRet)
				{
				...
				//上传是否下载模板成功信息
				bRet = g_toolTrade.UpLoadProGrameStatus(g_Config.m_strHttpUrl.GetBuffer(0), m_nMode, bRet == TRUE ? 3 : 4);
				if (bRet)
				{
		...
		{
			LOG(LOGTYPE_DEBUG, LOG_NAME_DEBUG, "RMQ_DealCustomMsg", "收到上送设备状态消息");
			SetEvent(m_hDviceStatusEvent);
		...
		else if (vct[0].CompareNoCase("DEVICECONTROLL") == 0)//设备控制
		{
			if (vct[1].CompareNoCase(g_Config.m_strDeviceCode) == 0)
			{
				...
				else if (vct[2].CompareNoCase("SCREENCUT") == 0)//截屏上传
				{
					...
					BOOL bRet = g_toolTrade.UpLoadPic(g_Config.m_strHttpUrl.GetBuffer(0), strBase64Pic, strPicName);
					if (bRet)
					{
...
DWORD CWeChatPrinterDlg::ReSignThreadContent(LPVOID pParam)
{
	while (FALSE == m_bExit)
	{
		...
		{
			SHOW_TIPS("正在签到");
			BOOL bRet = g_toolTrade.Login(g_Config.m_strHttpUrl.GetBuffer(0), m_iNextBitSpace, m_strLastSignTime, m_OnlineTime,m_strOrgCode,m_strDeviceType);
			if (FALSE == bRet)
			{
...
DWORD CWeChatPrinterDlg::DeviceStatusThreadContent(LPVOID pParam)
{
	while (FALSE == m_bExit)
	{
		...
		{
			//上传设备状态
			BOOL bRet = g_toolTrade.UpLoadDeviceStatus(g_Config.m_strHttpUrl, g_Config.m_strDeviceCode.GetBuffer(0));
			if (bRet)
			{
				CTime tmBegin = CTime::GetCurrentTime();
				CString str_Begin = tmBegin.Format("%Y%m%d%H%M%S");
				LOG(LOGTYPE_DEBUG, LOG_NAME_DEBUG, "DeviceStatusThreadContent", "[设备状态上传成功！][%s]", str_Begin);
			}
			else
			{
				CTime tmBegin = CTime::GetCurrentTime();
				CString str_Begin = tmBegin.Format("%Y%m%d%H%M%S");
				LOG(LOGTYPE_DEBUG, LOG_NAME_DEBUG, "DeviceStatusThreadContent", "[设备状态上传失败！][%s][%s]", str_Begin, g_toolTrade.GetLastErr());
			}
		}
...
DWORD CWeChatPrinterDlg::HeartBeatThreadContent(LPVOID pParam)
{
	while (FALSE == m_bExit)
	{
		...
		if (dwResult == WAIT_OBJECT_0)
		{
			...
			if (TRUE == m_bExit) return TRUE;
			BOOL bRet = g_toolTrade.HeartBeat(g_Config.m_strHttpUrl.GetBuffer(0), m_iNextBitSpace, m_strLastSignTime, m_OnlineTime);
			if (FALSE == bRet)
			{
	...
	{
		{
			...
			if (nResult == 1)
			{
				LOG(LOGTYPE_DEBUG, LOG_NAME_DEBUG, "OnCommand", "用户输入密码：12342314 后程序退出");
				EndDialog(TRUE);
			}
		}
	}
